...
struct RsPointXYZIRT {
    PCL_ADD_POINT4D;
    float intensity; // uint8_t intensity;
    uint16_t ring = 0;
    double timestamp = 0;
...
POINT_CLOUD_REGISTER_POINT_STRUCT(RsPointXYZIRT,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)    //(uint8_t, intensity, intensity)
...
public:
    ...
    {
        pub_points_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("/velodyne_points", 1);
        ...
        if (input_type == "XYZI") {
            ...
            RCLCPP_ERROR(this->get_logger(), "Unsupported input pointcloud type: %s. Currently only support XYZI and XYZIRT.",
                        input_type.c_str());
            throw std::runtime_error("Invalid input type");
        }

        RCLCPP_INFO(this->get_logger(), "Listening to /rslidar_points ......");
        RCLCPP_INFO(this->get_logger(),"Start publish /velodyne_points");
    }
...
private:
    ...
    void publish_points(T &new_pc, const sensor_msgs::msg::PointCloud2 &old_msg) {
        ...
        pub_points_->publish(pc_new_msg);
    }
...
int main(int argc, char **argv) {
    ...
    auto node = std::make_shared<RsConverter>(input_type, output_type);
    rclcpp::spin(node);
    rclcpp::shutdown();
